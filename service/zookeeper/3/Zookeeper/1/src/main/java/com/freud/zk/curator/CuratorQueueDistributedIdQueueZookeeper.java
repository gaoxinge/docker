...
public class CuratorQueueDistributedIdQueueZookeeper {
    ...
    static class Task implements Callable<Void> {
        ...
        public Void call() throws Exception {
            ...
            DistributedIdQueue<String> queue = QueueBuilder.builder(client, new StringQueueConsumer(index), new StringQueueSerializer(), path).buildIdQueue();
            queue.start();
            if (index == 4) {
                Thread.sleep(3 * SECOND);
                for (int i = 0; i < 20; i++) {
                    queue.put("message " + i, String.valueOf(i));
                }
                queue.remove("10");
            }
            return null;
